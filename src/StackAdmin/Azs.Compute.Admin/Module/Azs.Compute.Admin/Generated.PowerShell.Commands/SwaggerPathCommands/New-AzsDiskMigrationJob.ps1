...
<#
.SYNOPSIS
    Create a migration job for managed disks
...
function New-AzsDiskMigrationJob
{
    ...
    param(    
        ...
        [Parameter(Mandatory = $false, ParameterSetName = 'DiskMigrationJobs_Create')]
        [System.String]
        $Location,
    ...
    if ('DiskMigrationJobs_Create' -eq $PsCmdlet.ParameterSetName -or 'InputObject_DiskMigrationJobs_Create' -eq $PsCmdlet.ParameterSetName -or 'ResourceId_DiskMigrationJobs_Create' -eq $PsCmdlet.ParameterSetName) {
        ...
        $diskList = New-Object System.Collections.Generic.List[Microsoft.AzureStack.Management.Compute.Admin.Models.Disk]
        $Disks| %{$diskList.Add($_)}
        $TaskResult = $ComputeAdminClient.DiskMigrationJobs.CreateWithHttpMessagesAsync($Location, $MigrationId, $TargetShare, $diskList)
    } else {
